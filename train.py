...
if __name__ == "__main__":
    ...
    def make_env(env_id=args.env_id):
        ...
        if env_id == "reacher":
            ...
            if "gen" not in env_id and "sg" not in env_id:
                ...
                args.goal_start_idx = 0
                args.goal_end_idx = 2
            elif "gen" not in env_id and "sg" in env_id and "sgd" not in env_id:
                from envs.ant_maze_sg import AntMazeSG
                env = AntMazeSG(
                    backend="spring",
                    exclude_current_positions_from_observation=False,
                    terminate_when_unhealthy=True,
                    maze_layout_name=env_id[4:], # TODO: we only want the big maze here for now, change it later for more single goals
                    goal_location=json.loads(args.goal_location)
                )
                args.obs_dim = 29
                args.goal_start_idx = 0
                args.goal_end_idx = 2
            elif "gen" not in env_id and "sgd" in env_id:
                from envs.ant_maze_sgd import AntMazeSGD
                env = AntMazeSGD(
                    backend="spring",
                    exclude_current_positions_from_observation=False,
                    terminate_when_unhealthy=True,
    ...
    # TODO: MODIFY CRITIC OPTIMIZATION FOR SUCCESSFUL VS NONSUCCESSFUL TRAJECTORIES
    @jax.jit
